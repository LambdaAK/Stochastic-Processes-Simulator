...
function mergeTransitions(transitions: { from: string; to: string; rate: number }[]) {
  const map = new Map<string, number>()
  for (const t of transitions) {
    const key = `${t.from}\t${t.to}`
    map.set(key, (map.get(key) ?? 0) + t.rate)
  }
  return Array.from(map.entries()).map(([key, rate]) => {
...
export function CtmcGraph({ chain, width = DEFAULT_SIZE, height = DEFAULT_SIZE }: Props) {
          ...
          return (
            <g key={`${e.from}-${e.to}-${i}`}>
              ...
              <text x={labelX} y={labelY} className={styles.edgeLabel} textAnchor="middle" dominantBaseline="middle">
                {e.rate === Math.round(e.rate) ? e.rate : e.rate.toFixed(2)}
              </text>
            </g>
          )
        })}
